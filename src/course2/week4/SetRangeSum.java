...
public class SetRangeSum {
    ...
    void erase(int x) {
        VertexPair split = split(root, x);
        Vertex left = split.left;
        Vertex right = split.right;
        // x was the biggest number
        if (right == null) {
            return;
        }
        if (right.right == null) {
    ...
    boolean find(int x) {
        ...
        if (vertexPair.left == null) {
            return false;
        }
        return vertexPair.left.key == x;
    }

    long sum(int from, int to) {
        VertexPair leftMiddle = split(root, from);
        if (leftMiddle.right == null) {
            //if split.right is null, no actual splitting has been done;
            return 0;
        }
        Vertex left = leftMiddle.left;
        Vertex middle = leftMiddle.right;
        VertexPair middleRight = split(middle, to + 1);
        if (middleRight.right == null) {
            return 0;
        }
